...
def detect_emotion(image):
    ...
    for (x,y,w,h) in faces:
        ...
        if np.sum([roi_gray])!=0:
            ...
            label_position=(x,y)
            cv2.putText(frame,label,label_position,cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1)
        else:
            cv2.putText(frame,'No Face Found',(10,10),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,0),1)
...
while cap.isOpened():
    ...
    for (x,y,w,h) in faces:
        ...
        if np.sum([roi_gray])!=0:
            ...
            label_position=(x,y)
            cv2.putText(frames,label,label_position,cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1)
        else:
            cv2.putText(frames,'No Face Found',(10,10),cv2.FONT_HERSHEY_SIMPLEX,2,(255,255,0),2)
